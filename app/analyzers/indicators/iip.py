...
class IIP(IndicatorUtils):
    def analyze(
        ...
        dataframe["candle_length"] = np.abs(
            dataframe["high"] * 100000 - dataframe["low"] * 100000
        )
        dataframe.ta.zscore(
            close=dataframe["candle_length"],
            length=period_count,
            std=hot_thresh,
            append=True,
        )
        ...
        dataframe["is_hot"] = (dataframe["iip"] >= hot_thresh) & (
            dataframe["close"] > dataframe["open"]
        )
        dataframe["is_cold"] = (dataframe["iip"] >= hot_thresh) & (
            dataframe["close"] < dataframe["open"]
        )
